...
function getBinaryName() {
  ...
  if (platform === "linux" && arch === "arm64") {
    throw new Error(
      "Herozion: Linux ARM64 is not supported by the npm binary installer yet.\n" +
      "Use: pip install herozion  OR  download from:\n" +
      "  https://github.com/Herozion/scanner-releases/releases/latest"
    );
  }
...
function _isAllowedRedirectHost(host) {
  if (_ALLOWED_REDIRECT_HOSTS.has(host)) {
    return true;
  }
  // GitHub release binaries are served from various *.githubusercontent.com CDNs.
  // Allow the suffix so new CDN hostnames do not break installs.
...
function _validateRedirectUrl(location) {
  ...
  if (!_isAllowedRedirectHost(host)) {
    throw new Error(
...
async function main() {
  ...
  let bundledHash = "";
  if (fs.existsSync(bundledChecksumsPath)) {
    try {
      const bundled = JSON.parse(fs.readFileSync(bundledChecksumsPath, "utf8"));
      bundledHash = (bundled[binaryName] || "").toLowerCase().trim();
      if (!bundledHash) {
        console.warn(`herozion: WARNING — no bundled hash entry for ${binaryName}.`);
      }
    } catch (parseErr) {
      console.warn(`herozion: WARNING — invalid checksums.json: ${parseErr.message}`);
    }
  } else {
    console.warn(
      "herozion: WARNING — bin/checksums.json missing from npm package; " +
      "falling back to remote CHECKSUMS.sha256 verification."
    );
  }
  ...
  try {
      ...
      if (remoteHash) {
        if (actualHash !== remoteHash) {
          ...
          throw new Error(
            ...
            `  Got               : ${actualHash}\n` +
            "The downloaded binary does not match the GitHub Releases CHECKSUMS.sha256 file."
          );
        }
        remoteVerified = true;
      }
    } catch (checksumErr) {
      // If bundled verification already passed, remote failure is non-fatal.
      if (!bundledVerified) {
        throw checksumErr;
      }
    ...
    // Fail closed: require at least one independent verification source.
    if (!bundledVerified && !remoteVerified) {
      fs.unlinkSync(tmpPath);
      throw new Error(
        `Cannot verify ${binaryName}: no bundled hash in package and remote checksum unavailable.\n` +
        "Installation aborted to prevent supply-chain risk.\n" +
        `Download manually from: ${BASE_URL}/${binaryName}`
      );
    }

    // Verification passed — move binary into place
    ...
    const verifiedBy = bundledVerified && remoteVerified
      ? "bundled + remote SHA-256"
      : bundledVerified
        ? "bundled SHA-256"
        : "remote SHA-256";
    ...
    console.error(`herozion: ERROR — could not install binary: ${err.message}`);
    console.error("Try reinstalling with scripts enabled:");
    console.error("  npm install herozion --foreground-scripts");
    console.error(`Or download manually from: ${BASE_URL}/${binaryName}`);
    process.exit(1);
  }
}
