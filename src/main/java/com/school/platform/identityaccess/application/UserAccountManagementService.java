...
public class UserAccountManagementService implements IUserAccountManagementService {
    ...
    public UserAccountSummaryDTO updateAccount(Long id, UserAccountSummaryDTO dto) {
        ...
        if (dto.getPassword() != null && !dto.getPassword().isBlank()) account.setPassword(passwordEncoder.encode(dto.getPassword()));
        ...
        else if (dto.getRoleCode() != null && !dto.getRoleCode().isBlank()) account.setRoles(Set.of(findActiveRole(dto.getRoleCode())));
    ...
    private UserAccount findAccount(Long id) {
        return userAccountRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Utilisateur non trouve"));
    }
