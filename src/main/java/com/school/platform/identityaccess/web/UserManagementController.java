...
@RestController
@RequestMapping({"/user-accounts", "/users"})
...
public class UserManagementController {
    ...
    public ResponseEntity<UserAccountSummaryDTO> getAccountById(@PathVariable Long id) {
        return ResponseEntity.ok(usersService.getAccountById(id));
    }

    @PostMapping
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<UserAccountSummaryDTO> createUser(@Valid @RequestBody RegisterRequest registerRequest) {
        return ResponseEntity.status(HttpStatus.CREATED).body(usersService.registerAccount(registerRequest));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<UserAccountSummaryDTO> updateAccount(@PathVariable Long id, @Valid @RequestBody UserAccountSummaryDTO dto) {
        return ResponseEntity.ok(usersService.updateAccount(id, dto));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> deactivateAccount(@PathVariable Long id) {
        usersService.deactivateAccount(id);
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<UserAccountSummaryDTO> updateStatus(@PathVariable Long id, @RequestBody Map<String, String> payload) {
        return ResponseEntity.ok(usersService.updateStatus(id, payload.get("status")));
    }

    @PutMapping("/{id}/role/{roleId}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<UserAccountSummaryDTO> assignRole(@PathVariable Long id, @PathVariable Long roleId) {
        return ResponseEntity.ok(usersService.assignRole(id, roleId));
    }

    @DeleteMapping("/{id}/role")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<UserAccountSummaryDTO> clearDynamicRole(@PathVariable Long id) {
