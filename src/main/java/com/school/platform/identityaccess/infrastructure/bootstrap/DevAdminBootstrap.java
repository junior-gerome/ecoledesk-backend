...
public class DevAdminBootstrap implements ApplicationRunner {
    ...
    public void run(ApplicationArguments arguments) {
        ...
        log.info("=== DevAdminBootstrap ===");
        log.info("Bootstrap activé : {}", enabled);
        log.info("Email administrateur : {}", adminEmail);
        ...
        if (!StringUtils.hasText(adminEmail)
            ...
            throw new IllegalStateException(
                    "Bootstrap administrateur activé, "
                            + "mais l'email ou le mot de passe n'est pas configuré.");
        }
        ...
        log.info("Recherche du rôle ADMIN...");

        Role admin = roleRepository
                .findByCodeIgnoreCase("ADMIN")
                .orElseThrow(() -> new IllegalStateException(
                        "Le rôle ADMIN n'existe pas dans la base. "
                                + "Crée d'abord le rôle ADMIN via Flyway."));
